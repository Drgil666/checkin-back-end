package com.example.demo.mapper.impl;

import com.example.demo.mapper.CheckInDao;
import com.example.demo.pojo.CheckIn;
import org.springframework.data.mongodb.core.MongoTemplate;
import org.springframework.data.mongodb.core.query.Criteria;
import org.springframework.data.mongodb.core.query.Query;
import org.springframework.stereotype.Component;

import javax.annotation.Resource;
import java.util.List;

/**
 * @author Gilbert
 * @date 2020/9/25 16:46
 */
@Component("CheckInDaoImpl")
public class CheckInDaoImpl implements CheckInDao {
    @Resource
    private MongoTemplate mongoTemplate;

    /**
     * 创建签到
     *
     * @param checkIn 签到
     * @return 带有id的签到
     */
    @Override
    public CheckIn createCheckIn(CheckIn checkIn) {
        mongoTemplate.insert(checkIn);
        return checkIn;
    }

    /**
     * 更新签到
     *
     * @param checkIn 签到
     * @return 影响行数
     */
    @Override
    public CheckIn updateCheckIn(CheckIn checkIn) {
        return mongoTemplate.save(checkIn);
    }

    /**
     * 通过用户id和课程id获取签到记录
     *
     * @param userId   用户id
     * @param courseId 课程id
     * @return 签到列表
     */
    @Override
    public List<CheckIn> getCheckInListByUserAndCourse(Integer userId, Integer courseId) {
        Query query = new Query();
        query.addCriteria(Criteria.where("userId").is(userId).and("courseId").is(courseId));
        return mongoTemplate.find(query, CheckIn.class);
    }

    /**
     * 通过id查找签到记录
     *
     * @param id 签到id
     * @return 签到记录
     */
    @Override
    public CheckIn getCheckInById(Integer id) {
        Query query = new Query();
        query.addCriteria(Criteria.where("_id").is(id));
        return mongoTemplate.findOne(query, CheckIn.class);
    }

    /**
     * 批量删除签到
     *
     * @param id 要删除的列表
     * @return 影响的行数
     */
    @Override
    public long deleteCheckIn(List<Integer> id) {
        Query query = new Query();
        query.addCriteria(Criteria.where("_id").in(id));
        return mongoTemplate.remove(query, CheckIn.class).getDeletedCount();
    }
}
